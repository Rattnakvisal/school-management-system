document.addEventListener('DOMContentLoaded', () => {
    const root = document.querySelector('[data-student-notification-root]');
    if (!root) {
        return;
    }
    ...
    const renderNotifications = (notifications) => {
        ...
        if (!Array.isArray(notifications) || notifications.length === 0) {
            list.innerHTML = '<div id="student-notif-empty" class="px-4 py-8 text-center text-sm text-slate-500">No notifications yet.</div>';
            return;
        }
        ...
        list.innerHTML = notifications.map((notification) => {
            ...
            const url = String(notification?.url || '').trim() || '/student/notices';
                ...
                <a href="${escapeHtml(url)}" class="block px-4 py-3 hover:bg-slate-50">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-1 text-[11px] text-slate-400">${time}</div>
                        </div>
                    </div>
                </a>
            `;
    ...
    const poll = async () => {
        try {
            ...
            console.error('Unable to poll student notifications.', error);
        }
    };
