...
<body class="min-h-full overflow-x-hidden bg-slate-100 text-slate-800" data-loading-shell>
    <div id="admin-swirling-loader" class="admin-swirling-loader" role="status" aria-live="polite"
        aria-label="Loading staff panel">
    ...
    <div x-data="staffShell()" x-init="init()" @keydown.escape.window="closeAll()" class="min-h-screen">
        ...
        <aside
            ...
            <div class="flex h-20 shrink-0 items-center justify-between border-b border-slate-200/80 px-5">
                ...
                <div class="flex items-center gap-2">
                    <button type="button"
                        class="hidden h-10 w-10 items-center justify-center rounded-2xl text-slate-600 transition hover:bg-white hover:text-slate-900 hover:shadow-[0_10px_20px_-16px_rgba(15,23,42,0.35)] focus:outline-none focus:ring-4 focus:ring-indigo-100 lg:inline-flex"
            ...
            <nav class="nav-scrollbar flex-1 space-y-6 overflow-y-auto px-3 py-5">
                @php
                    ...
                    $sections = [
                        [
                            'title' => 'Main',
                            'items' => [$item('staff.dashboard', 'Dashboard', 'layout-dashboard')],
                        ],
                        [
                            'title' => 'Management',
                            'items' => [
                        ...
                        <div class="space-y-1.5">
                            @foreach ($section['items'] as $l)
                                <a href="{{ route($l['route']) }}"
                                    ...
                                    @if ($l['active'])
                                        <span class="absolute bottom-2 left-0 top-2 w-1 rounded-r-full bg-white"></span>
                                    @endif
        ...
        function staffShell() {
            return {
                ...
                toggleSidebar() {
                    ...
                    localStorage.setItem('staff_sidebar_collapsed', this.collapsed ? '1' : '0');
                }
            }
        }
    </script>
