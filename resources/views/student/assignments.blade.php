...
@section('page')
    ...
    <div class="student-stage space-y-6">
                    ...
                    <div class="admin-page-header__title-row flex items-start gap-3">
                        <span class="admin-page-header__icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                                ...
                                <path
                                    d="M8 3.5h8A2.5 2.5 0 0 1 18.5 6v14l-3-1.75L12.5 20l-3-1.75L6.5 20V6A2.5 2.5 0 0 1 9 3.5Z" />
                                <path d="M9.5 8h5" />
                                <path d="M9.5 11.5h5" />
                                <path d="M9.5 15h3" />
                            </svg>
                        ...
                        <div>
                            ...
                            <h1 class="admin-page-title text-3xl font-black tracking-tight sm:text-4xl">Your Assignment
                                Board</h1>
                        </div>
                    </div>
            ...
            <section
                ...
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-xs font-medium text-slate-500">Choose a file from the row, then send it to your
                            teacher.</p>
                    </div>
                    <span
                        class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm">
                                        ...
                                        <tr id="assignment-{{ $assignment->id }}" x-data="{ detailOpen: false }"
                                            ...
                                            <td class="px-3 py-3 align-top">
                                                <div class="flex items-start gap-3">
                                                    <span
                                                        class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-50 text-indigo-600 ring-1 ring-indigo-100">
                                                    ...
                                                    <div class="min-w-0">
                                                        <div
                                                            class="max-w-[280px] whitespace-normal font-semibold leading-5 text-slate-800">
                                                            {{ $assignment->title }}
                                                        </div>
                                                        <div class="mt-1 max-w-[280px] truncate text-xs text-slate-400">
                                                            ID
                                                            #{{ str_pad((string) $assignment->id, 6, '0', STR_PAD_LEFT) }}
                                            ...
                                            <td class="px-3 py-3 align-top text-slate-600">
                                                <div class="flex max-w-[220px] flex-col gap-1">
                                                    <span
                                                        ...
                                                        <span
                                                            class="truncate font-semibold text-indigo-700">{{ $subjectLabel }}</span>
                                                    </span>
                                            ...
                                            <td class="px-3 py-3 align-top text-slate-600">
                                                <div class="max-w-[160px] truncate">
                                                    {{ $assignment->teacher?->name ?? 'Teacher' }}</div>
                                            </td>
                                            <td class="whitespace-nowrap px-3 py-3 align-top text-slate-500">
                                                <span>{{ $dueAt ? $dueAt->format('M d, Y') : 'No deadline' }}</span>
                                                @if ($dueAt)
                                                    <div class="mt-1 text-xs text-slate-400">{{ $dueAt->diffForHumans() }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="px-3 py-3 align-top">
                                                <span
                                                    class="inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-xs font-semibold {{ $statusClass }}">
                                                    <span
                                                        class="h-2 w-2 rounded-full {{ $isSubmitted ? 'bg-emerald-500' : ($statusLabel === 'Overdue' ? 'bg-rose-500' : ($statusLabel === 'Due soon' ? 'bg-amber-500' : 'bg-slate-400')) }}"></span>
                                                    {{ $statusLabel }}
                                                </span>
                                            </td>
                                            <td class="px-3 py-3 align-top text-slate-600">
                                                @if ($isSubmitted)
                                                    @php
                                                        ...
                                                        if ($fileSize > 0) {
                                                            ...
                                                            $fileSizeLabel =
                                                                rtrim(rtrim(number_format($displaySize, 1), '0'), '.') .
                                                                ' ' .
                                                                $units[$unitIndex];
                                                        }
                                                    @endphp
                                                    <button type="button" @click="detailOpen = true"
                                                        ...
                                                        <span
                                                            class="truncate font-semibold text-emerald-700">{{ $submission->submission_file_name ?: 'Submitted file' }}</span>
                                                    </button>
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="min-w-0">
                                                                    ...
                                                                    <p class="mt-1 text-xs text-slate-500">
                                                                        {{ $assignment->title }} - 1 file received.</p>
                                                                </div>
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <div class="min-w-0">
                                                                            <div class="flex flex-wrap items-center gap-2">
                                                                                <span
                                                                                    class="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-bold text-indigo-700 ring-1 ring-indigo-100">Student</span>
                                                                                <span
                                                                                    class="text-sm font-black text-slate-900">{{ auth()->user()?->name ?? 'Student' }}</span>
                                                                            </div>
                                                                            <div class="mt-1 text-xs text-slate-400">
                                                                            ...
                                                                            <div class="mt-3">
                                                                                <a href="{{ \Illuminate\Support\Facades\Storage::url($submission->submission_file_path) }}"
                                                                                    ...
                                                                                    <span
                                                                                        class="truncate">{{ $submission->submission_file_name ?: 'Submitted file' }}</span>
                                                                                </a>
                                                                            </div>
                                                                        </div>
                                                                        <div
                                            ...
                                            <td class="whitespace-nowrap px-3 py-3 align-top">
                                                <form method="POST"
                                                    action="{{ route('student.assignment.submit', $assignment) }}"
                                                    enctype="multipart/form-data"
                                                    class="js-student-assignment-submit-form flex flex-nowrap items-center justify-end gap-2"
                                                    data-assignment="{{ $assignment->title }}" novalidate
                                                    x-data="{ fileName: '' }">
                                                    @csrf
                                                    <input id="assignment_submission_{{ $assignment->id }}"
                                                        name="submission_file" type="file" class="sr-only" required
                                                    ...
                                                    <label for="assignment_submission_{{ $assignment->id }}"
                                                        ...
                                                        <svg class="h-4 w-4 text-indigo-600" viewBox="0 0 24 24"
        ...
        @else
            <section
                class="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center shadow-sm">
                <div class="text-xl font-black text-slate-900">No assignments yet</div>
                <p class="mt-2 text-sm text-slate-500">
                    When a teacher posts a new assignment for you, it will show up here and an alert notification will be
                    sent.
                </p>
            </section>
        @endif
            ...
            document.querySelectorAll('.js-student-assignment-delete-form').forEach((form) => {
                form.addEventListener('submit', (event) => {
                    ...
                    const confirmation = hasSwal ?
                        window.Swal.fire(options).then((result) => Boolean(result && result
                            .isConfirmed)) :
