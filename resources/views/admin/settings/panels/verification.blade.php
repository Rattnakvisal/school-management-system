                    <section data-settings-panel="verification" class="hidden">
                        <div
                            class="rounded-2xl border border-slate-200 bg-slate-50/70 p-6 dark:border-slate-700 dark:bg-slate-800/40">
                            <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Verification</h2>
                            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">This section is reserved for
                                account verification and
                                security checks.</p>
                        </div>
                    </section>
