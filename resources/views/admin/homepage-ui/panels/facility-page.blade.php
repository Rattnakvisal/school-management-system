<section data-settings-panel="facility-page" class="hidden space-y-6">
    @if ($homePageReady ?? false)
        <form method="POST" action="{{ route('admin.settings.facility-page.update') }}" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div
                ...
                <div class="px-6 pb-6 pt-7 sm:px-7">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                Image, badge, headline, and description shown in the Campus and Facilities section.
                            </p>
                        </div>
                    ...
                    <div class="mt-5 grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
                        <div>
                            <img id="facility_image_preview" src="{{ $facilityImage }}" alt="Facility section preview"
                                class="h-52 w-full rounded-2xl border border-slate-200 object-cover shadow-sm">

                            <input id="facility_image_input" name="facility[image]" type="file" accept="image/*"
                                class="hidden">

                            <input id="remove_facility_image" name="facility[remove_image]" type="hidden"
                                value="0">
                ...
                <div class="px-6 pb-6 pt-7 sm:px-7">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                Cards displayed inside the Campus and Facilities section.
                            </p>
                        </div>
                    ...
                    <div class="mt-4 grid gap-3 xl:grid-cols-2">
                        @for ($i = 0; $i < $facilityLimit; $i++)
                            @php
                                ...
                                $facilityTitle = old('facility_cards.' . $i . '.title', $facilityItem?->title ?? '');
                                ...
                                $facilityIcon = old('facility_cards.' . $i . '.icon', $facilityItem?->icon ?? '');
                                ...
                                <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <div data-home-card-actions class="flex flex-wrap items-center gap-2">
                                        ...
                                        <label
                                            ...
                                            <input type="hidden" name="facility_cards[{{ $i }}][active]"
                                                value="0">

                                            <input type="checkbox" name="facility_cards[{{ $i }}][active]"
