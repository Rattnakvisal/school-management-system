...
@section('page')
    ...
    <div class="teacher-stage admin-staff-page space-y-6">
            ...
            <section x-data="{
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-slate-500">Choose the account role and staff page access.</p>
                    </div>
                ...
                <form id="create-admin-staff-form-panel" method="POST" action="{{ route('admin.admin-staff.store') }}"
                    ...
                    @if ($hasStaffPermissionsColumn ?? false)
                        @php
                            ...
                            $selectedCreateStaffPermissions = is_array($selectedCreateStaffPermissions) ? $selectedCreateStaffPermissions : [];
                        @endphp
                        <div x-data="{ staffAccessOpen: false }"
                            ...
                            <button type="button" @click="staffAccessOpen = !staffAccessOpen"
                                ...
                                <span class="min-w-0">
                                    <span class="block text-xs font-black uppercase tracking-wide text-slate-500">Staff page access</span>
                                    <span class="mt-1 block text-[11px] leading-5 text-slate-500">Used only when role is Staff. Leave all unchecked for no admin pages.</span>
                                </span>
                            ...
                            <div x-show="staffAccessOpen" x-cloak x-transition class="mt-3 max-h-72 overflow-y-auto pr-1">
                                <div class="grid gap-2">
                                    @foreach ($staffPermissionOptions as $permissionKey => $permissionOption)
                                        <label class="flex gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
                                            <input type="checkbox" name="staff_permissions[]" value="{{ $permissionKey }}"
                                                ...
                                                class="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-indigo-600">
                                            <span class="min-w-0">
                                                <span class="block text-xs font-bold text-slate-700">{{ $permissionOption['label'] }}</span>
                                                <span class="block text-[11px] leading-4 text-slate-500">{{ $permissionOption['description'] }}</span>
                                            </span>
                                        </label>
                                    @endforeach
                                </div>
                            ...
                            <div
                                ...
                                <form method="POST" action="{{ route('admin.admin-staff.update', $account) }}"
                                    ...
                                    <div class="flex-1 space-y-4 overflow-y-auto overscroll-contain px-5 pb-4 pt-1">
                                        ...
                                        @if ($hasStaffPermissionsColumn ?? false)
                                            @php
                                                $accountStaffPermissions = \App\Support\StaffPermissions::permissionsFor($account);
                                                $accountPermissionsWereSaved = $account->getAttribute('staff_permissions') !== null;
                                            ...
                                            <div x-data="{ staffAccessOpen: false }"
                                                ...
                                                <button type="button" @click="staffAccessOpen = !staffAccessOpen"
                                                    ...
                                                    <span class="min-w-0">
                                                        <span class="block text-xs font-black uppercase tracking-wide text-slate-500">Staff page access</span>
                                                        <span class="mt-1 block text-[11px] leading-5 text-slate-500">Used only when this account role is Staff.</span>
                                                    </span>
                                                    <span class="flex shrink-0 items-center gap-2">
                                                        <span class="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-slate-500 ring-1 ring-slate-200">
