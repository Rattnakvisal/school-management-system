...
<section class="{{ $gridClass }}">
    @foreach ($cards as $index => $card)
        @php
            ...
            $progressText =
                (string) ($card['progressText'] ??
                    ($visibleTotal > 0
                        ? number_format($cardActive) . ' of ' . number_format($visibleTotal) . ' complete'
                        : 'No records yet'));
        ...
        <div class="{{ $revealClass }} {{ $floatClass }} min-h-[132px] rounded-[26px] border border-white/80 bg-white/90 p-5 shadow-[0_24px_55px_-36px_rgba(78,85,135,0.55)] backdrop-blur dark:border-slate-700/50 dark:bg-slate-800/50 dark:shadow-[0_24px_55px_-36px_rgba(0,0,0,0.55)]"
            ...
            <div class="mt-5 flex items-end gap-1 text-2xl font-black text-slate-950 dark:text-slate-100">
                <span>{{ $displayActive }}</span>
                @unless ($hideTotal)
                    <span class="pb-0.5 text-base font-extrabold text-slate-300 dark:text-slate-500">/
                        {{ $displayTotal }}</span>
                @endunless
            </div>
            <div class="mt-1 text-sm font-bold text-slate-600 dark:text-slate-400">{{ $card['label'] ?? 'Total' }}
            </div>
            ...
            <div class="mt-4 h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
